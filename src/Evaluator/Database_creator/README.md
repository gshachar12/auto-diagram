...
## Building a gold sample without hand-writing JSON

`gold_sample_template.xlsx` — a fillable Excel template (Instructions / Info / Actors / Claims sheets,
dropdowns for `severity` and `gold_grounding_status`, one example row per sheet). Fill in the yellow cells,
then convert:
...
**Note on the example row**: it's skipped by fixed row position (row 2), not by matching its content —
an earlier version matched by ID value and silently dropped real data when a real entry happened to reuse
a plausible ID like `"kali_entity"` (confirmed directly while testing). Your data always starts at row 3.
