...
def build_step_evidence_map(steps_json: str, packets):
    """
...
def extract_relevant_packets(
    pcap_bytes, max_packets=250, filtering_limit=3, display_filter="ip or arp"
):
    ...
    """
    relevant = []
    print("filtering_limit:", filtering_limit)
            ...
            for pkt in cap:
                ...
                try:
                    ...
                    if fingerprint_key not in fingerprint_tracker:
                        ...
                        if stats["count"] <= filtering_limit:
                            relevant.append({
                                "id": int(pkt.number),
                                "timestamp": pkt.sniff_time.isoformat(),
                                "protocol": pkt.highest_layer,
                            ...
                            omitted = stats["count"] - filtering_limit
...
def parse_with_indices(pcap_file, mode=""):
    ...
    return packets_data
def llm_preprocessing(packets_data, max_tokens=MAX_PCAP_TOKENS):
    """
    Prepares the packet data for LLM processing by tokenizing and ensuring it fits within the specified token limit.

    Args:
        packets_data (list): List of dictionaries containing packet information.
        max_tokens (int): Maximum number of tokens allowed for the LLM input.

    Returns:
        str: A string representation of the packet data suitable for LLM input.
    """ 
    
    # Convert the list of packet dictionaries into a single string representation
    packets_str = "\n".join([f"Packet #{p['id']}: {p['summary']}" for p in packets_data])
    
    # Tokenize the string using the provided tokenizer
    tokens_list = tokens.tokenize(packets_str)
    
    # Check if the token count exceeds the maximum allowed tokens
    if len(tokens_list) > max_tokens:
        st.warning(f"Token count ({len(tokens_list)}) exceeds the maximum allowed ({max_tokens}). Truncating data.")
        # Truncate the token list to fit within the limit
        tokens_list = tokens_list[:max_tokens]
    
    # Reconstruct the string from the truncated token list
    processed_str = tokens.detokenize(tokens_list)
...
def prompt(pcap_file_name, pcap_file, mode=""):
    ...
    if mode == "full" or mode == "extract":
        ...
        packets_data =  extract_relevant_packets(pcap_file, max_packets=200, filtering_limit=3)
